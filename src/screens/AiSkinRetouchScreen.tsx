...
const Header: React.FC<{ onBack: () => void; onDownload: () => void; canDownload: boolean; }> = ({ onBack, onDownload, canDownload }) => (
    <header className="flex-shrink-0 bg-[#2c2c2c] text-white flex items-center justify-between px-4 h-14 border-b border-gray-700/50 z-20">
        <div className="flex items-center gap-4">
            ...
            <h1 className="font-semibold text-lg">Retoque de Pele com IA</h1>
        </div>
...
export const AiSkinRetouchScreen: React.FC<{ onBack: () => void; onEdit: (imageUrl: string) => void; }> = ({ onBack, onEdit }) => {
    ...
    const handleRetouch = async (imageUrl: string) => {
        ...
        try {
            ...
            const prompt = `Faça um retoque de pele fotorrealista nesta imagem. Suavize a pele para reduzir manchas, acne e rugas finas, mas mantenha a textura natural da pele. Uniformize o tom da pele. Não altere as características faciais, cabelo, roupas ou fundo da pessoa. O resultado deve parecer um retrato profissionalmente retocado.`;
    ...
    const renderMainContent = () => {
        if (isProcessing) {
             return (
                <div className="flex flex-col items-center justify-center text-center p-4">
                    ...
                    <p className="text-gray-300 text-sm mt-1">A IA está aperfeiçoando a pele.</p>
                </div>
            );
        }
        ...
        return (
            <div className="w-full max-w-2xl text-center">
                ...
                <p className="text-gray-400 mb-6">Obtenha uma pele perfeita em suas fotos com um único clique.</p>
