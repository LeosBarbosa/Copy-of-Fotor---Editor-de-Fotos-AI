...
export const AiMagicEditScreen: React.FC<{
  ...
  const handleImprovePrompt = async () => {
      ...
      try {
          ...
          const response = await ai.models.generateContent({
              model: AI_TEXT_MODEL,
              contents: { parts: [{ text: `Given the user's request to edit an image: "${prompt}", rewrite it as a more detailed and effective prompt for an image generation AI. The new prompt should be descriptive and clear, focusing on visual changes. Only return the improved prompt text, nothing else.` }] },
          });
      ...
      <main className="flex-1 min-h-0 flex items-center justify-center p-4">
        {isGenerating ? (
          <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-gray-300">Aplicando sua edição mágica...</p>
          </div>
