...
export const ImageExpandScreen: React.FC<{ onBack: () => void; onEdit: (imageUrl: string) => void; }> = ({ onBack, onEdit }) => {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const prompt = `Expanda o canvas desta imagem para uma proporção de ${aspectRatio}, preenchendo de forma inteligente as novas áreas para combinar com o estilo e conteúdo existentes da imagem. O conteúdo da imagem original deve ser perfeitamente preservado no centro. Crie uma cena coesa e natural.`;
        ...
        <div className="flex flex-col h-screen bg-[#1F1F1F] text-white overflow-hidden">
            ...
            <header className="flex-shrink-0 z-10 p-4">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-semibold text-lg drop-shadow-md">Expansão de Imagem IA</h1>
                </div>
            </header>
            ...
            <main className="flex-1 min-h-0 flex flex-col items-center justify-center p-4">
                {isProcessing ? (
                    <div className="text-center">
                        ...
                        <p className="font-semibold">Expandindo sua imagem...</p>
                    </div>
                ) : processedImage ? (
                    ...
                    <div className="text-center max-w-2xl">
                       <Expand size={48} className="mx-auto text-blue-400 mb-4"/>
                        <h2 className="text-3xl font-bold mb-2">Expansão de Imagem com IA</h2>
                        <p className="text-gray-400 mb-6">Amplie os limites de suas fotos com o preenchimento generativo de IA.</p>
            ...
            <footer className="flex-shrink-0 z-10 p-4">
                ...
                {originalImage && (
                    <div className="max-w-lg mx-auto flex items-center gap-4 bg-black/30 backdrop-blur-sm p-4 rounded-xl">
                        <div>
                            <h4 className="font-semibold text-sm mb-2 text-gray-300">Proporção da Tela</h4>
                            ...
                            {processedImage ? (
                                <>
                                 ...
                                 <button onClick={handleProcess} disabled={isProcessing} className="bg-blue-600 font-bold py-3 px-4 rounded-lg hover:bg-blue-700 disabled:bg-gray-500 self-stretch">
                                    {isProcessing ? 'Expandindo...' : 'Expandir'}
                                </button>
                            )}
                        </div>
                    </div>
