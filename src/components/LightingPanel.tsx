...
import { ArrowLeft, Sun, Moon, CloudSun, Sunrise, Zap } from 'lucide-react';
...
export const LightingPanel: React.FC<LightingPanelProps> = ({ onBack, image, onApply }) => {
    ...
    const handleApplyLighting = async () => {
        ...
        try {
            ...
            const prompt = `Analyze this image. Apply the following lighting adjustment: ${presetPrompt} Intensity: ${intensityDesc}. Goal: A professional, natural, and high-quality result. Maintain the original composition.`;
        ...
        <div className="w-[calc(100vw-5rem)] sm:w-80 bg-[#2c2c2c]/90 backdrop-blur-lg shadow-2xl h-full flex flex-col flex-shrink-0 relative">
             {isProcessing && (
                <div className="absolute inset-0 bg-black/70 z-10 flex flex-col items-center justify-center text-center p-4">
                    ...
                    <h3 className="text-white font-semibold text-lg">Ajustando iluminação...</h3>
                    <p className="text-gray-300 text-sm mt-1">A IA está processando a luz.</p>
                </div>
            )}
            <header className="flex items-center gap-4 p-4 border-b border-gray-700/50 flex-shrink-0">
                ...
                <h2 className="font-semibold text-sm text-white">Iluminação Inteligente</h2>
            </header>
             ...
             <div className="flex-1 p-4 space-y-6 flex flex-col">
                 ...
                 <div className="text-center mb-2">
                     ...
                     <h3 className="text-white font-semibold mb-1">Controle de Luz</h3>
                     <p className="text-xs text-gray-400">
                         Selecione um modo e ajuste a intensidade.
                     </p>
                 </div>

                 {/* Presets Grid */}
                 <div>
                    <label className="text-xs font-semibold text-gray-300 mb-2 block uppercase tracking-wider">Modo de Correção</label>
